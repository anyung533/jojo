...
function getList() {
  return new Promise(resolve => {
    $.get(listUrl('getTaskList'), async (err, resp, data) => {
      try {
        if (err) {
...
function getFinishCount() {
  return new Promise(resolve => {
    $.get(listUrl('getTaskFinishCount'), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          if (safeGet(data)) {
            const result = JSON.parse(data);
			//console.log(`getTaskFinishCount：${JSON.stringify(result)}`)
			if (result.code == 1) {
				console.log(`\n任务完成状态：${result.content.finishCount}/${result.content.maxTaskCount}`)
				if (result.content.finishCount == result.content.maxTaskCount) {
					$.taskStop = true
					console.log(`\n今日任务已完成！`)
				}
			} else {
				console.log(`\n获取任务完成状态失败：${JSON.stringify(result)}`)
			}
          }
        }
      } catch (e) {
...
function doTask(body) {
  return new Promise(resolve => {
    $.post(taskUrl('saveTaskRecord',body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          if (safeGet(data)) {
            const result = JSON.parse(data);
			//console.log(`doTask：${JSON.stringify(result)}`)
			if (result.code == 1) {
				console.log(`\n获取任务 ${$.taskName} 成功 ---`)
				$.uid = result.content.uid
				$.tt = result.content.tt
				await $.wait(parseInt($.watchTime)*1000)
				await getReward()
			} else {
				console.log(`\n获取任务 ${$.taskName} 失败：${JSON.stringify(result)}`)
			}
          }
        }
      } catch (e) {
	...
	$.post(taskUrl('saveTaskRecord',body), async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            const result = JSON.parse(data);
			//console.log(`getReward：${JSON.stringify(result)}`)
			if (result.code == 1) {
...
//listUrl
function listUrl(functionId) {
  return {
    url: `https://ifanli.m.jd.com/rebateapi/task/`+functionId,
    headers: {
	  'Cookie': cookie,
