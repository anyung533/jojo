...
function runTimes(thisShareCode) {
  return new Promise((resolve, reject) => {
    $.get({
      url: `https://api.jdsharecodde.xyz/api/runTimes0509?activityId=farm&sharecode=${thisShareCode}`
    }, (err, resp, data) => {
      if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://api.jdsharecodde.xyz/api/farm/50`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
