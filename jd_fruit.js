...
async function jdFruit() {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      // 报告运行次数
      $.get({
        url: `https://cdn.nz.6lu/api/runTimes?activityId=farm&sharecode=${$.farmInfo.farmUserPro.shareCode}`,
        headers: {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://cdn.nz.lux/api/farm/${randomCount}`, headers: {'Host': 'api.sharecode.ga'}, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
