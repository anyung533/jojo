...
!(async () => {
  ...
  $.authorTuanList = await getAuthorShareCode('https://raw.githubusercontent.com/Aaronj-lv/updateTeam/master/shareCodes/jd_zz.json');
  if (!$.authorTuanList) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aaron-flv/updateTeam@master/shareCodes/jd_zz.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorTuanList = await getAuthorShareCode('https://cdn.jsdelivrf.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_zz.json') || [];
  }
  // await getRandomCode();
